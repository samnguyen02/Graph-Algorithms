...
std::list<value_type<T>> dijkstrasAlgorithm(const WeightedGraph<T>& graph, vertex_type<T> initial_node, vertex_type<T> destination_node)
{
    ...
    while (!q.empty()) {
        ...
        for (auto pair = graph.at(u).begin(); pair != graph.at(u).end(); pair++) {
            auto v = pair->first;
            if (s.find(v) != s.end()) {
                continue;
            }
...
std::list<value_type<T>> topologicalSort(const WeightedGraph<T>& graph)
{
    ...
    for (auto v_it = graph.begin(); v_it != graph.end(); v_it++) {
        if (indegrees[v_it->first] == 0) {
            q.push(v_it->first);
        }
    }
    ...
    while (!q.empty()) {
        ...
        for (auto adj_it = graph.at(v).begin(); adj_it != graph.at(v).end(); adj_it++) {
            indegrees[adj_it->first]--;
            if (indegrees[adj_it->first] == 0) {
                q.push(adj_it->first);
            }
        }
    }
